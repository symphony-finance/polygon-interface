...
// returns null on errors
export function useSymphonyContract(withSignerIfPossible = true) {
  const { chainId, library, account } = useActiveWeb3React()

  return useMemo(() => {
    try {
      return getSymphonyContract(chainId, library, withSignerIfPossible ? account : undefined)
    } catch (e) {
      return null
    }
...
export function useWETHGatewayContract(withSignerIfPossible = true) {
  const { chainId, library, account } = useActiveWeb3React()
