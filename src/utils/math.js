...
export function getMedian(values) {
  ...
  if (values.length % 2 === 0) {
    ...
    return [
      ...
      sortMap.map(element => (indices.includes(element) ? ethers.BigNumber.from(0.5) : ethers.BigNumber.from(0)))
    ]
  } else {
    const middle = Math.floor(values.length / 2)
    return [sortedValues[middle], sortMap.map(element => (element === middle ? ethers.BigNumber.from(1) : ethers.BigNumber.from(0)))]
  }
}

export function getMean(values, _weights) {
  const weights = _weights ? _weights : values.map(() => ethers.BigNumber.from(1))
  ...
  const denominator = weights.reduce((accumulator, currentValue) => accumulator.plus(currentValue), ethers.BigNumber.from(0))
